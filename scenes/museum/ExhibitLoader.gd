...
func init(museum: Node3D, config: Dictionary) -> void:
	...
	# 	if hint_manager:
	# 		if hint_manager.has_signal("hints_loaded"):
	# 			if not hint_manager.hints_loaded.is_connected(_on_hints_loaded):
	# 				hint_manager.hints_loaded.connect(_on_hints_loaded)
	# 				print("ExhibitLoader: Connected to HintManager.hints_loaded signal")
	# 		else:
	# 			print("ExhibitLoader: WARNING - HintManager doesn't have hints_loaded signal")
	# 	else:
...
func _on_exit_added(exit: Hall, doors: Array, backlink: bool, new_exhibit: Node3D, hall: Hall) -> void:
	...
	# Hint system disabled - no backlink special handling
	# Backlink system disabled - use normal door assignment
	if doors.size() > 0:
...
# Hint system disabled - remaining functions are no longer needed
# The code below is kept for reference but not called

func _unused_hintsPlaceholder() -> void:
	pass  # Placeholder to avoid empty file issues
