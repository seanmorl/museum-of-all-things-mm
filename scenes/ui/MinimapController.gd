...
## cycles three minimap display styles with the M key.
##
...
## MODES (M key cycles):
##   0 = OFF
##   1 = Parchment  — aged paper, sepia-tinted texture
##   2 = Modern     — clean card, full-colour texture
##   3 = WikiGraph  — article graph, no texture needed
##   4 = Full Map   — fog-of-war schematic, reveals as you explore
...
const ParchmentScript := preload("res://scenes/ui/ParchmentMinimap.gd")
const ModernScript    := preload("res://scenes/ui/ModernMinimap.gd")
const WikiGraphScript := preload("res://scenes/ui/WikiGraphMinimap.gd")
const FullMapScript   := preload("res://scenes/ui/FullMapMinimap.gd")
...
func _ready() -> void:
	...
	var parchment : Control = ParchmentScript.new()
	parchment.name    = "ParchmentMinimap"
	parchment.visible = false
	add_child(parchment)

	var modern : Control = ModernScript.new()
	modern.name    = "ModernMinimap"
	modern.visible = false
	add_child(modern)

	var wiki : Control = WikiGraphScript.new()
	wiki.name    = "WikiGraphMinimap"
	wiki.visible = false
	add_child(wiki)

	var fullmap : Control = FullMapScript.new()
	fullmap.name    = "FullMapMinimap"
	fullmap.visible = false
	add_child(fullmap)

	_views = [parchment, modern, wiki, fullmap]
