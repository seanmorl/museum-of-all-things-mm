extends Control
## SimpleMinimap - Redesigned 2D topological minimap (matches moat-ui.html).
##
...
func _draw_overlay() -> void:
	...
	var outer_r : float = COMPASS_RADIUS * _zoom
	var inner_r : float = (COMPASS_RADIUS - 5) * _zoom
	...
	# "N" label
	var north_tip := compass_pos + Vector2(sin(heading), -cos(heading)) * (outer_r + 6.0 * _zoom)
	var font := _overlay.get_theme_default_font()
	_overlay.draw_string(font, north_tip - Vector2(4.0 * _zoom, 4.0 * _zoom), "N", HORIZONTAL_ALIGNMENT_CENTER, -1, int(10.0 * _zoom), _col_north)
...
func _animate_out() -> void:
	...
	tw.chain().tween_callback(func(): _panel.visible = false; _panel.modulate.a = 1.0; _panel.scale = Vector2.ONE)
