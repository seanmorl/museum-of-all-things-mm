...
static func apply() -> void:
	...
	if not museum:
		print("[FlyingArtworkEvent] Failed: Museum not found")
		return
	
	# Get current exhibit
	var current_room = museum.get("current_room") if museum.has_method("get_current_room") else "Unknown"
	print("[FlyingArtworkEvent] Applied: Artwork flying off walls in '%s'!" % current_room)
	...
	if items_found == 0:
		print("[FlyingArtworkEvent] No grouped items found, searching by type...")
	...
	print("[FlyingArtworkEvent] Made %d items fly!" % items_found)
...
static func end() -> void:
	...
	_flying_items.clear()
	print("[FlyingArtworkEvent] Ended: Artwork returned to walls")
