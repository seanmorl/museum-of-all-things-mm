...
static func apply() -> void:
	...
	if not _world_env or not is_instance_valid(_world_env):
		print("[FogEvent] Failed to apply: WorldEnvironment not found")
		return

	var env = _world_env.environment
	if not env:
		print("[FogEvent] Failed to apply: No environment in WorldEnvironment")
		return
	...
	print("[FogEvent] Applied: Dense fog enabled (density=%f)" % env.fog_density)
...
static func end() -> void:
	...
	print("[FogEvent] Ended: Fog restored")
