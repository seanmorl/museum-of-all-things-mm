...
func _find_python() -> void:
	...
	for path in paths:
		if FileAccess.file_exists(path):
			...
			print("[PiperService] Found Python: ", path)
			return
	
	# Fallback to PATH
	python_executable = "python"
	print("[PiperService] Using Python from PATH")
...
func _scan_voices() -> void:
	...
	if not dir:
		print("[PiperService] Could not open voices directory: ", VOICES_DIR)
		return
	...
	while file_name != "":
		if file_name.ends_with(".onnx"):
			...
			print("[PiperService] Found voice model: ", voice_name)
		file_name = dir.get_next()
	
	dir.list_dir_end()
	
	if _voice_models.size() == 0:
		print("[PiperService] WARNING: No voice models found in ", VOICES_DIR)
		print("[PiperService] Download voices with: python -m piper.download_voices")
	else:
		print("[PiperService] Total voices found: ", _voice_models.size())
		current_voice = _voice_models[0]
		print("[PiperService] Default voice set to: ", current_voice)
...
func _check_piper() -> void:
	...
	if exit_code == 0:
		is_ready = true
		print("[PiperService] ✓ Piper TTS is ready")
		
		if _voice_models.size() > 0:
			current_voice = _voice_models[0]
			print("[PiperService] Using voice: ", current_voice)
	else:
		is_ready = false
		print("[PiperService] ✗ Piper TTS not found")
		print("[PiperService] Install with: pip install piper-tts")
...
func speak(text: String) -> void:
	...
	if not is_ready:
		print("[PiperService] Not ready - Piper not installed")
	...
	if _generation_active:
		...
		_pending_text = text
		print("[PiperService] Generation in progress, queuing text")
		return
	
	print("[PiperService] Speaking: ", text.substr(0, 50))
...
func _synthesize(text: String) -> AudioStreamWAV:
	...
	if FileAccess.file_exists(cache_file):
		print("[PiperService] Using cached audio")
		return _load_wav(cache_file)
	
	# Generate new audio
	print("[PiperService] Generating speech...")
	...
	if exit_code == 0 and FileAccess.file_exists(cache_file):
		print("[PiperService] Generated ", cache_file)
		return _load_wav(cache_file)
	else:
		print("[PiperService] Generation failed: ", exit_code)
		if output.size() > 0:
			print("[PiperService] Error: ", "".join(output))
		return null
...
func _play_audio(stream: AudioStreamWAV) -> void:
	...
	speech_started.emit()
	print("[PiperService] Playback started")


func _on_playback_finished() -> void:
	"""Called when audio playback completes"""
	print("[PiperService] Playback finished")
...
func set_voice(voice_name: String) -> bool:
	...
	if voice_name in _voice_models:
		...
		print("[PiperService] Voice set to: ", voice_name)
		return true
	else:
		print("[PiperService] Voice not available: ", voice_name)
		return false
...
func clear_cache() -> void:
	...
	if dir:
		...
		while file_name != "":
			...
			file_name = dir.get_next()
		dir.list_dir_end()
		print("[PiperService] Cache cleared")
